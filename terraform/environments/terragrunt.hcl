...
remote_state {
  ...
  generate = {
    path      = "backend.tf"
    # "overwrite" (not "overwrite_terragrunt") is required because
    # terraform/stack/backend.tf is copied into the Terragrunt cache and was
    # not written by Terragrunt, so "overwrite_terragrunt" refuses to replace it.
    if_exists = "overwrite"
  }
  config = {
...
# Declares all providers used by the stack modules (aws + random for RDS password).
generate "provider" {
  path      = "provider.tf"
